...
> **核心前提**：要走這條路，HTML 必須從第一行就按下面 4 條約束寫。**不是寫完再轉**——事後補救會觸發 2-3 小時重工（2026-04-20 期權私董會專案實測踩雷）。
>
...
| `1920px × 1080px` | 20″ × 11.25″ | 自訂 | ❌ 非標尺寸，投影後字型顯得異常小 |
...
**安全限制**：PPTX 匯出只接受本地檔案或 data URI 圖片；`scripts/image-source-guard.js` 會在交給 PptxGenJS 前拒絕遠端/custom-scheme 圖片，以及 ICNS、HEIF/HEIC/AVIF、JXL 等格式。請先把這些圖片轉換為 PNG、JPEG、WebP 或 SVG，不要繞過這個檢查處理不可信素材。

---

## 合併文字框（`data-pptx-merge`）

**預設行為**：HTML 裡每個 `<p>`/`<h1>`-`<h6>` 在 PPTX 裡都是**獨立文字框**。卡片裡寫 3 個 `<p>` → PPT 裡 3 個文字框摞著，編輯時不能整段回車換行加段，得逐個改字號/對齊。
...
這裡的 doctrine 是：**使用者給的是設計意圖，你負責翻譯成合規實作**。不是讓使用者去學 4 條硬約束然後自己重寫。

改寫時的遵循原則：
- **保留**：色彩系統（主色/輔色/中性色）、資訊層級（標題/副標題/正文/註解）、核心文案、layout 骨架（上中下 / 左右分欄 / 網格）、頁面節奏
- **降級**：CSS 漸變 → 純色或 flex 分段、web component → 段落級 HTML、複雜 SVG → 簡化的 `<img>` 或純色幾何、陰影 → 刪除或降為極弱、自訂字型 → 向系統字型靠齊
...
理解這點後，**不要期待工具變聰明** —— 是 HTML 寫法要相容於 PPTX 格式，不是反過來。
