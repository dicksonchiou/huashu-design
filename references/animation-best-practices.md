...
> 色彩決策走 §1.a 核心資產協議（從品牌 spec 擷取）或「設計方向顧問」
...
下面 §1-§8 的技術規則，是這套身分在具體場景的**執行手段**——不是獨立規則清單。

- 遇到規則沒覆蓋的場景 → 回到 §0，用**身分**判斷，不要瞎猜
...
真實世界的物體有品質、有慣性、有彈性、會溢位。Anthropic 三支片子的「高階感」根源，
...
**具體色值決策**走 §1.a 核心資產協議（從品牌 spec 擷取）或「設計方向顧問」
...
## 6.5 · 導演稿的視覺密度條款（B00實戰教訓，2026-07-17）

**只寫敘事+運鏡的導演稿，會收到線框稿。** B00階躍b-roll實測：v1導演稿把六幕的敘事、時間軸、鏡頭運動寫得很全，實作agent交付的動畫動效全部合格、check全綠——但視覺是「三塊素色暗塊+大字」的示意圖水平，被導演一票退回（「太簡單太無聊」）。agent在沒有密度標準時，永遠按最省幾何交差。

**導演稿（或任何動畫brief）必須顯式包含三樣**：

1. **視覺密度標準**：每個畫面細節元素的量級要求（骨架UI內容行、圖例、紋理、次級元素），以及一句可執行的驗收表述，如「任意暫停一幀，和參照標杆擺在一起不丟人」
2. **參照標杆**：指向一個具體的已有成品（同項目舊動畫/A系列/某個demo），構件工藝直接搬，不讓agent憑空發明
3. **全域性氛圍層清單**：地面線錨定、構件軟陰影、紙面/背景紋理、hero跟班或同類人格化小元素、靜止構件的idle微動（呼吸/游標/微推進）——「空曠無聊感」的主要解藥是這一層，不是主元素本身

**修復路徑也有定式**：動效骨架（時間軸/運鏡/morph路徑/字卡時機）與視覺工藝（構件/密度/氛圍）是兩層，驗收被退回時先問清是哪層的問題——動效過關就只做re-skin，不重編舞。
...
**呼叫順序**：
1. 先看 SKILL.md 工作流程 Step 3 的form推導五問（決定敘事角色和視覺溫度）
