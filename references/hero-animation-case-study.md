...
1. **素材規模 ≥ 20 張，最好 30+**
   少於 20 張 Ripple 會顯得"空"——48 格裡每格都在動才有密度感。v9 用了 48 格 × 32 張圖（迴圈填充）。
...
**不只是 opacity——同時 desaturate + darken**。這讓前景 overlay 的色彩"跳出來"，而不只是"變亮一點"。
...
- 比全螢幕大 logo 更高階——做品牌的人知道，品牌簽名不需要喊
- 在 GIF 被截圖分享時仍留下歸屬訊號
...
**❌ 產品演示（要展示功能的）**：Gallery 讓每一張都一閃而過，觀眾記不住任何一個功能。改用「單一畫面 focus + tooltip 標註」。
...
- 原始 HTML（v6 + 音訊整合版）：`www.huasheng.ai/huashu-design-hero/index.html`
