...
這條是**非協商**的。每次寫`const styles = {...}`都必須replace成specific命名，否則多元件載入時全端出錯。
...
部分原生 design-agent 環境（如 Claude.ai Artifacts）有免設定的 `window.claude.complete`，但大部分 agent 環境（Claude Code / Codex / Cursor / Trae / etc.）本地裡**沒有**。

如果你的 HTML 原型需要呼叫 LLM 做 demo（比如做個聊天 interface），兩個選項：

### 選項A：不真調，用mock

Demo場景推薦。寫一個假helper，回傳預設的response：
```jsx
window.claude = {
...
### 選項B：真調Anthropic API（不推薦，僅限本地演示）

需要API key，使用者必須在HTML裡填入自己的key才能跑。**永遠不要把key硬編碼在HTML裡**。

⚠️ 安全邊界：這個方案只適合本地`file://`開啟、用完即關的演示。key會留在DOM/記憶體裡——
**不要部署這個頁面、不要帶著填了key的頁面截圖/錄影傳播**。生產場景一律走本地proxy後端轉發，
瀏覽器端不碰key。預設優先選項A/C（完全不需要key）。
...
**注意**：瀏覽器直接調 Anthropic API 會遇到 CORS 問題。如果使用者給你的預覽環境不支援 CORS bypass，這條路不通。此時使用選項 A mock，或者告訴使用者需要一個 proxy 後端。

### 選項 C：用 agent 側的 LLM 能力生成 mock 資料

如果只是本地演示用，可以在目前 agent 會話裡臨時呼叫該 agent 的 LLM 能力（或使用者裝的 multi-model 類 skill）先生成 mock 回應資料，再硬編碼寫進 HTML。這樣 HTML 執行時完全不依賴任何 API。
