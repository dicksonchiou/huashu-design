...
**跨 agent 環境支援**：某些 design-agent 原生環境（如 Claude.ai Artifacts）依賴 host 的 postMessage 把 tweak 值回寫原始碼做持久化。本 skill 採用**純前端 localStorage 方案**——效果一致（重新整理保留狀態），但持久化發生在瀏覽器 localStorage 而不是原始碼檔案。這個方案在任何 agent 環境（Claude Code / Codex / Cursor / Trae / etc.）都能工作。
...
一個設計的Tweaks面板**最多5-6個**選項。再多就變成"設定頁面"，失去了快速探索variations的意義。
...
標記塊在 localStorage 方案裡**無作用**（只是個普通註釋），但在支援原始碼回寫的 host 裡會被讀取，實作原始碼級持久化。加上這個對目前環境無害，同時保持向前相容。
