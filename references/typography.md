...
> **這不是字型清單，是配對與排版的推理規則。** `design-styles.md` 已經給了 60 種風格各自的字型名；本文回答的是「為什麼這樣配」「拿到任意內容怎麼推匯出字號/行長/字重」。目標：同一個風格標籤，落到不同內容上，能推匯出不同的排印結果，而不是每次都抄同一套字號。
>
...
1. **內容型別** → 長文閱讀 / 資料密集 / 行銷大字 / UI 介面，決定音階比例和正文字號
...
**流式字號寫法**（display 檔必用，避免大螢幕死板小螢幕溢位）：

```css
/* clamp(最小值, 首選值, 最大值)：首選值 = 基礎rem + 可視區係數 */
...
為什麼 display 脫離音階：hero 巨字是版面元素不是文字層級，它的尺寸由「佔可視區幾成」決定，用 vw 推導比用音階推導更合理。
...
|---|------------------------|----------|------|------|
| 1 | Newsreader + Geist | 形式對比：螢幕顯示最佳化的過渡襯線，x-height 高、與 Geist 咬合好；**Fraunces 的正牌替代字型** | 安靜 | Google Fonts / Vercel 官方倉庫 |
...
| 6 | Schibsted Grotesk + Source Serif 4 | 反轉結構：grotesque 當 display、襯線當正文，媒體感；**Space Grotesk 氾濫後的替代字型**（挪威 Schibsted 報業定製開源，帶新聞血統） | 中性 | Google Fonts |
...
| 思源黑體（Noto Sans SC） | 黑體 | 中文界的 Inter：可靠、無表情，當預設正文沒錯但沒個性 | 全溫度備援 | Google Fonts，OFL |
| 霞鶩文楷 | 楷體 | 手寫溫度、親切，適合文藝/教育/個人部落格正文與引文 | 安靜·暖 | GitHub lxgw/LxgwWenKai，OFL |
| 霞鶩新晰黑 | 黑體 | 比思源黑更瘦更透氣的螢幕顯示黑，正文久讀不累 | 安靜 | GitHub lxgw/LxgwNeoXiHei |
...
- **字重對比是主武器**：思源宋 Heavy 900 壓 Light 300，同一字型兩個極端字重同一個畫面，比換字型更有張力且零載入成本
...
**中文字型載入**（單檔案 5-15MB，直接引全量會毀掉首個畫面）：
...
- `font-display: swap` 保底，中文字型下載慢，白畫面等字型是最差體驗
