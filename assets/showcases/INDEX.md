...
| 代號 | 流派 | 風格名稱 | 視覺氣質 |
|------|------|---------|---------|
| **Pentagram** | 資訊建築派 | Pentagram / Michael Bierut | 黑白克制、瑞士網格、強字型層級、#E63946 紅色強調 |
...
|---|------|------|-----------|-------|--------|
| 1 | 官方帳號封面 | 1200×510 | `cover/cover-pentagram` | `cover/cover-build` | `cover/cover-takram` |
...
| 7 | SaaS 登陸頁 | 1440×900 | `website-saas/saas-pentagram` | `website-saas/saas-build` | `website-saas/saas-takram` |
| 8 | 開發人員文件 | 1440×900 | `website-devdocs/devdocs-pentagram` | `website-devdocs/devdocs-build` | `website-devdocs/devdocs-takram` |
...
```
「這是 Pentagram 風格做官方帳號封面的效果 → [展示 cover/cover-pentagram.png]」
「Takram 風格做 PPT 資料頁是這種感覺 → [展示 ppt/ppt-takram.png]」
```

### 場景比對優先順序
1. 使用者需求的場景有精確符合 → 直接展示對應場景
2. 無精確符合但型別相近 → 展示最接近的場景（如「產品官網」→ 展示 SaaS 登陸頁）
3. 完全不符合 → 跳過預製範例，直接進 Phase 3.5 現場產生

### 橫向對比展示
同一場景的 3 個風格適合並排展示，幫助使用者直觀比較：
- 「這是同一個官方帳號封面，分別用 3 種風格實作的效果」
- 展示順序：Pentagram（理性克制）→ Build（奢華極簡）→ Takram（柔和溫暖）

## 內容詳情

### 官方帳號封面（cover/）
- 內容：Claude Code Agent 工作流程 — 8 個並行 Agent 架構
...
- Takram：紙張紋理 + 小圓形頭像 + 髮絲細分隔線 + 不對稱佈局

### AI 導覽站（website-ai-nav/）
- 內容：AI Compass — 500+ AI 工具目錄
- Pentagram：方角搜尋框 + 編號工具清單 + 大寫分類標籤
